...
pygame.display.set_caption('Basestation')
icon = pygame.image.load('MAVRIC.png')
pygame.display.set_icon(icon)

BSExit = False
mainMenu = True

#messages/text function define
...
autonomous_map_N_label_draw_pos = (250, 430)
autonomous_map_draw_pos = (250, 600)
...
#Parameters to watch on screen
#Wheels
def writeWheels():
    message_to_screen("WHEEL DATA",white, -440,"small",250)
    message_to_screen("Left Wheels: %f "%(drive_l),white, -400,"small",250)
    message_to_screen("Right Wheels: %f "%(drive_r),white, -380,"small",250)
#arm
def writeArms():
    message_to_screen("ARM DATA",white, -340,"small",250)
    message_to_screen("Base Rotation: %f "%(shoulder_r_axis),white, -300,"small",250)
    message_to_screen("Base Pitch: %f"%shoulder_p_axis,white, -280,"small",250)
    message_to_screen("Elbow Pitch: %f"%elbow_p_val,white, -260,"small",250)
    message_to_screen("Claw Rotation: %f"%claw_r_axis,white, -240,"small",250)
    message_to_screen("Claw Pitch: %f"%claw_p_axis,white, -220,"small",250)
    message_to_screen("Claw Actuation: %f"%claw_a_val,white, -200,"small",250)
#temperature
def writeTemp():
    message_to_screen("TEMPERATURE DATA",white, -160,"small",250)
    message_to_screen("Temperature: "+rover.temperature,white, -120,"small",250)
#actuator (ask about)
...
def writeTelem(theta,phi):
    pygame.draw.line(screen,red, (520,700), (520+200*math.cos(theta),700+200*math.sin(theta)), 10)
    pygame.draw.line(screen,red, (520+200*math.cos(theta),700+200*math.sin(theta)), (520+200*math.cos(theta)+250*math.cos(phi),700+200*math.sin(theta)+250*math.sin(phi)), 10)
    message_to_screen("ARM TELEMETRY",white, -60,"small",250)
...
while not BSExit:
    ...
    while mainMenu == True:
        ...
        #main menu graphics
        screen.fill(red)
        message_to_screen("Launch",black, -20,"large")
        message_to_screen("(T)",green, 20, "large")
    ...
    for event in pygame.event.get():
        ...
        #get shoulder and wrist commands from joysticks
                ...
                if is_arm_stick(get_stick(event.joy)):
                    if event.axis == 0:
                        ...
                        theta = theta+event.value*-1*2*3.14/180
        ...
        #get elbow command from arm controller d-pad
                ...
                if is_arm_stick(get_stick(event.joy)):
                    elbow_p_val = int(event.value[1] * 100)
                    phi = phi+1*2*3.14/180
    ...
    if emergency_stop:
        ...
        label = warn_font.render(e_stop_message, 1, (200, 255, 255))
    ...
    #ALL READOUTS WILL BE DRAWN HERE
    #Controller
