...
class Pathing():
    ...
    def check_intersection(self, WP_r, WP_theta, O_r, O_theta, O_R):
        ...
        i_theta = m.degrees(m.atan(A_R/O_r))
    ...
    def get_path_points(self, WP_r, WP_theta, O_r, O_theta, O_R, dir_mod):
        # assume WP_theta = 0
        delta_theta = m.radians(abs(O_theta - WP_theta))
        A_R = self.a_mod*O_R+self.w/2

        # find PP1 and PP5
        D_R = A_R - O_r*m.sin(delta_theta)
        A_l = m.sqrt(m.pow(A_R+D_R, 2)-m.pow(A_R, 2))
        PP1_r = O_r*m.cos(delta_theta) - A_l
        PP5_r = O_r*m.cos(delta_theta) + A_l

        # find D1 and D2
        D1_r = m.sqrt(m.pow(D_R, 2)+m.pow(PP1_r, 2))
        D1_theta = m.atan(D_R/PP1_r)
        D2_r = m.sqrt(m.pow(D_R, 2)+m.pow(PP5_r, 2))
        D2_theta = m.atan(D_R/PP5_r)

        # find PP3
        PP3_r = m.sqrt(m.pow(D_R, 2)+m.pow(O_r*m.cos(delta_theta), 2))
        PP3_theta = m.atan(D_R/(O_r*m.cos(delta_theta)))

        # find PP2 and PP4
        A_theta = m.atan(A_l/A_R)
        PP2_r = m.sqrt(m.pow(O_r, 2)+m.pow(A_R, 2)-2*O_r*A_R*m.cos(m.pi/2-delta_theta-A_theta))
        PP2_theta = m.asin(m.sin(m.pi/2-delta_theta-A_theta)*A_R/PP2_r) - delta_theta
        PP4_r = m.sqrt(m.pow(O_r, 2)+m.pow(A_R, 2)-2*O_r*A_R*m.cos(m.pi/2-delta_theta+A_theta))
        PP4_theta = m.asin(m.sin(m.pi/2-delta_theta+A_theta)*A_R/PP4_r) - delta_theta

        # polor cords for all points
        D1_p = [D1_r, m.degrees(dir_mod*D1_theta + WP_theta)]
        D2_p = [D2_r, m.degrees(dir_mod*D2_theta + WP_theta)]
        PP1_p = [PP1_r, m.degrees(WP_theta)]
        PP2_p = [PP2_r, m.degrees(dir_mod*PP2_theta + WP_theta)]
        PP3_p = [PP3_r, m.degrees(dir_mod*PP3_theta + WP_theta)]
        PP4_p = [PP4_r, m.degrees(dir_mod*PP4_theta + WP_theta)]
        PP5_p = [PP5_r, m.degrees(WP_theta)]
        ...
        return D_R, A_R, A_theta, PP1, PP2, PP3, PP4, PP5
    ...
    def run(self):
        ...
        if self.obstacle_num == 0:
            ...
            if self.enter_point > g.pathpoint_num >= self.exit_point:
                self.referance_point = g.position
        ...
        for i in range(0, len(g.objects["distance"])):
            ...
            if intersects and o_r < g.prev_linear_error:
                ...
                g.pathpoints["position"].extend([pp1, pp2, pp3, pp4, pp5])
                g.pathpoints["linear"].extend([True, False, False, False, False])
                g.pathpoints["heading"].extend([ep_theta, ep_theta-mod*a_theta, ep_theta, ep_theta+mod*a_theta, ep_theta])
                g.pathpoints["radius"].extend([None, d_r, a_r, a_r, d_r])
                g.pathpoints["speed"].extend([80, 60, 60, 60, 60])

                self.enter_point = g.pathpoints["position"].index(pp1)
                self.exit_point = g.pathpoints["position"].index(pp5)
