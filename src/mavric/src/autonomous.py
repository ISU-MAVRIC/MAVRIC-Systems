...
def main():
    ...
    while 1:
        ...
        if True: #gps.good_fix
            ...
            pos_str = raw_input("current lat, lon: ").split(",")
            pos = [0,0]
            pos[0] = float(pos_str[0])
            pos[1] = float(pos_str[1])
            #head = gps.heading
            head = float(raw_input("heading: "))
            tgt = [0, 0]
        ...
        #print(str(linear_error))
        #print(str(angular_error))
        ...
        #publish command string ("D" + frac_to_cs(left_power) + frac_to_cs(right_power))
